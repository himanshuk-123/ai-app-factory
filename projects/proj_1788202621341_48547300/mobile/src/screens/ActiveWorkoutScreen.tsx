...
export const ActiveWorkoutScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.card}>
      <Text style={styles.cardTitle}>{"Active Workout Logger"}</Text>
      <Text style={styles.cardDesc}>{"High-contrast, low-friction interface for real-time set tracking, rest timer orchestration, and auto-regulated weight/rep dynamic adjustments."}</Text>
      ...
      {viewState === 'LOADING' && (
        <View style={[styles.stateBanner, styles.loadingBanner]}>
          ...
          <Text style={styles.loadingText}>⏳ {"Minimal translucent loading overlay with text 'Preparing dynamic session adjustments...'"}</Text>
        </View>
      )}

      {viewState === 'EMPTY' && (
        <View style={[styles.stateBanner, styles.emptyBanner]}>
          <Text style={styles.emptyText}>📭 {"Empty exercise log placeholder with prompt 'No exercises in this session. Tap + to add an exercise.'"}</Text>
        </View>
      )}

      {viewState === 'ERROR' && (
        <View style={[styles.stateBanner, styles.errorBanner]}>
          <Text style={styles.errorText}>⚠️ {"Inline notification badge 'Timer sound muted due to silent mode. Rest timer visible on screen.'"}</Text>
        </View>
      )}

      {viewState === 'SUCCESS' && (
        <View style={[styles.stateBanner, styles.successBanner]}>
          <Text style={styles.successText}>✅ {"Full-screen celebration modal with confetti effect, total volume summary, and 'Workout Saved' badge."}</Text>
        </View>
      )}
      ...
      {viewState === 'NORMAL' && (
        <View>
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoContent}>{"Effortlessly log sets, reps, and RPE during a workout while adhering to automated rest timing and load adjustments."}</Text>
          </View>

          <View style={styles.infoBox}>
            <Text style={styles.infoTitle}>📐 Layout Architecture</Text>
            <Text style={styles.infoContent}>{"Top fixed bar (timer, workout progress bar, finish button) + Scrollable stack of exercise log cards with interactive set rows + Bottom fixed rest timer controller strip."}</Text>
          </View>
