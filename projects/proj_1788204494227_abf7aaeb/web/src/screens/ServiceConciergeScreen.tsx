...
export const ServiceConciergeScreen: React.FC = () => {
  ...
  return (
    <div className="card">
      <div className="card-title">{"Service Menu & Concierge Preferences Screen"}</div>
      <p className="card-desc">{"Allows clients to select haircut and beard services while customizing bespoke pre-appointment amenities."}</p>
      ...
      {viewState === 'LOADING' && (
        <div className="state-banner loading">
          ⏳ {"Accordion skeletons and placeholder cards pulsing softly."}
        </div>
      )}

      {viewState === 'EMPTY' && (
        <div className="state-banner empty">
          📭 {"Alert: 'No services available for current selection. Please adjust criteria.'"}
        </div>
      )}

      {viewState === 'ERROR' && (
        <div className="state-banner error">
          ⚠️ {"Banner: 'Failed to lock service pricing. Please refresh.'"}
        </div>
      )}

      {viewState === 'SUCCESS' && (
        <div className="state-banner success">
          ✅ {"Concierge preferences confirmed checkmark toast."}
        </div>
      )}
          ...
          <div style={{ background: 'var(--bg-main)', padding: '20px', borderRadius: 'var(--radius)', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginTop: '4px', color: 'var(--text-main)' }}>
                {"Curate custom grooming services and specify personal lounge environment preferences (beverage, ambient lighting, music genre)."}
              </h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Architecture: {"Step progress header + segmented tab control (Services / VIP Concierge) + scrollable customization list + sticky subtotal footer"}
              </p>
            </div>
