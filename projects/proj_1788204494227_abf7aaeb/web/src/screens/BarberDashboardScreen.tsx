...
export const BarberDashboardScreen: React.FC = () => {
  ...
  return (
    <div className="card">
      <div className="card-title">{"Stylist Schedule & Client Intel Screen"}</div>
      <p className="card-desc">{"Barber-facing dashboard showing upcoming appointments, chair readiness status, and detailed VIP client preferences."}</p>
      ...
      {viewState === 'LOADING' && (
        <div className="state-banner loading">
          ⏳ {"Timeline skeleton slots with shimmering block lines."}
        </div>
      )}

      {viewState === 'EMPTY' && (
        <div className="state-banner empty">
          📭 {"Schedule card: 'No upcoming appointments for today. Enjoy your downtime.'"}
        </div>
      )}

      {viewState === 'ERROR' && (
        <div className="state-banner error">
          ⚠️ {"Red warning banner: 'Offline mode active. Modifications will sync when signal returns.'"}
        </div>
      )}

      {viewState === 'SUCCESS' && (
        <div className="state-banner success">
          ✅ {"Green check status icon on completed client slots."}
        </div>
      )}
          ...
          <div style={{ background: 'var(--bg-main)', padding: '20px', borderRadius: 'var(--radius)', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginTop: '4px', color: 'var(--text-main)' }}>
                {"Review daily schedule, inspect incoming client profile & concierge requests, and mark appointments as complete."}
              </h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Architecture: {"Top metrics summary bar (Daily Revenue, Appts Count) + timeline schedule feed + expandable client intel cards"}
              </p>
            </div>
