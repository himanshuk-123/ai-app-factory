...
export const VipReceiptScreen: React.FC = () => {
  ...
  return (
    <div className="card">
      <div className="card-title">{"VIP Digital Receipt & Pass Screen"}</div>
      <p className="card-desc">{"Presents a premium pass detailing booking information, concierge choices, directions, and instant check-in access QR code."}</p>
      ...
      {viewState === 'LOADING' && (
        <div className="state-banner loading">
          ⏳ {"Pass placeholder card with glowing gold outline animation."}
        </div>
      )}

      {viewState === 'EMPTY' && (
        <div className="state-banner empty">
          📭 {"Card state: 'No active VIP pass found.' with link to Lounge Showcase."}
        </div>
      )}

      {viewState === 'ERROR' && (
        <div className="state-banner error">
          ⚠️ {"Alert: 'Pass details synced locally offline. QR Code remains valid for entrance.'"}
        </div>
      )}

      {viewState === 'SUCCESS' && (
        <div className="state-banner success">
          ✅ {"Confetti particle emission (gold foil) on initial arrival from booking flow."}
        </div>
      )}
          ...
          <div style={{ background: 'var(--bg-main)', padding: '20px', borderRadius: 'var(--radius)', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginTop: '4px', color: 'var(--text-main)' }}>
                {"View reservation confirmation, save pass to Apple/Google Wallet, get salon directions, or check in upon arrival."}
              </h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Architecture: {"Central metallic gold VIP digital pass card + detailed summary breakdown below + dual floating action buttons"}
              </p>
            </div>
