...
export const SlotPickerScreen: React.FC = () => {
  ...
  return (
    <div className="card">
      <div className="card-title">{"Schedule & Time Slot Selector Screen"}</div>
      <p className="card-desc">{"Enables date and time slot selection synchronized with real-time room, chair, and master barber availability."}</p>
      ...
      {viewState === 'LOADING' && (
        <div className="state-banner loading">
          ⏳ {"Grid skeleton for time slot pills pulsing at 1.5s interval."}
        </div>
      )}

      {viewState === 'EMPTY' && (
        <div className="state-banner empty">
          📭 {"Illustration of clock: 'No available slots on this date. Tap to view next available date with Master Stylist.'"}
        </div>
      )}

      {viewState === 'ERROR' && (
        <div className="state-banner error">
          ⚠️ {"Modal dialog: 'This slot was just claimed by another VIP client. Please pick another time.'"}
        </div>
      )}

      {viewState === 'SUCCESS' && (
        <div className="state-banner success">
          ✅ {"Hold timer countdown initialized indicator in top header bar."}
        </div>
      )}
          ...
          <div style={{ background: 'var(--bg-main)', padding: '20px', borderRadius: 'var(--radius)', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginTop: '4px', color: 'var(--text-main)' }}>
                {"Select a convenient date and exact time slot that fits seamlessly into the client's high-profile schedule."}
              </h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Architecture: {"Header stepper + calendar date strip horizontal scroller + morning/afternoon/evening available slot grid + summary confirmation modal sheet"}
              </p>
            </div>
