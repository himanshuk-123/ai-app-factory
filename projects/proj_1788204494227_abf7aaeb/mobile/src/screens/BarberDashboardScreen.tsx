...
export const BarberDashboardScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.card}>
      <Text style={styles.cardTitle}>{"Stylist Schedule & Client Intel Screen"}</Text>
      <Text style={styles.cardDesc}>{"Barber-facing dashboard showing upcoming appointments, chair readiness status, and detailed VIP client preferences."}</Text>
      ...
      {viewState === 'LOADING' && (
        <View style={[styles.stateBanner, styles.loadingBanner]}>
          ...
          <Text style={styles.loadingText}>⏳ {"Timeline skeleton slots with shimmering block lines."}</Text>
        </View>
      )}

      {viewState === 'EMPTY' && (
        <View style={[styles.stateBanner, styles.emptyBanner]}>
          <Text style={styles.emptyText}>📭 {"Schedule card: 'No upcoming appointments for today. Enjoy your downtime.'"}</Text>
        </View>
      )}

      {viewState === 'ERROR' && (
        <View style={[styles.stateBanner, styles.errorBanner]}>
          <Text style={styles.errorText}>⚠️ {"Red warning banner: 'Offline mode active. Modifications will sync when signal returns.'"}</Text>
        </View>
      )}

      {viewState === 'SUCCESS' && (
        <View style={[styles.stateBanner, styles.successBanner]}>
          <Text style={styles.successText}>✅ {"Green check status icon on completed client slots."}</Text>
        </View>
      )}
      ...
      {viewState === 'NORMAL' && (
        <View style={{ gap: 12 }}>
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoTitle}>{"Review daily schedule, inspect incoming client profile & concierge requests, and mark appointments as complete."}</Text>
            <Text style={styles.infoContent}>Structure: {"Top metrics summary bar (Daily Revenue, Appts Count) + timeline schedule feed + expandable client intel cards"}</Text>
          </View>

          {/* Interactive UI Component Cards */}
          {["Barber Daily Overview Metric Bar","Timeline View with Client Avatars & Service Tags","VIP Client Preference Badge (e.g., 'Scotch: Macallan 12', 'Quiet Service Requested')","Chair Readiness Status Switch (Ready / In-Service / Cleaning)","Action Buttons: 'Notify Client Lounge Ready', 'Complete Cut'"].map((comp, idx) => {
