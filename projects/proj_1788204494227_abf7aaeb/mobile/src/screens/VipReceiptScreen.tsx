...
export const VipReceiptScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.card}>
      <Text style={styles.cardTitle}>{"VIP Digital Receipt & Pass Screen"}</Text>
      <Text style={styles.cardDesc}>{"Presents a premium pass detailing booking information, concierge choices, directions, and instant check-in access QR code."}</Text>
      ...
      {viewState === 'LOADING' && (
        <View style={[styles.stateBanner, styles.loadingBanner]}>
          ...
          <Text style={styles.loadingText}>⏳ {"Pass placeholder card with glowing gold outline animation."}</Text>
        </View>
      )}

      {viewState === 'EMPTY' && (
        <View style={[styles.stateBanner, styles.emptyBanner]}>
          <Text style={styles.emptyText}>📭 {"Card state: 'No active VIP pass found.' with link to Lounge Showcase."}</Text>
        </View>
      )}

      {viewState === 'ERROR' && (
        <View style={[styles.stateBanner, styles.errorBanner]}>
          <Text style={styles.errorText}>⚠️ {"Alert: 'Pass details synced locally offline. QR Code remains valid for entrance.'"}</Text>
        </View>
      )}

      {viewState === 'SUCCESS' && (
        <View style={[styles.stateBanner, styles.successBanner]}>
          <Text style={styles.successText}>✅ {"Confetti particle emission (gold foil) on initial arrival from booking flow."}</Text>
        </View>
      )}
      ...
      {viewState === 'NORMAL' && (
        <View style={{ gap: 12 }}>
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoTitle}>{"View reservation confirmation, save pass to Apple/Google Wallet, get salon directions, or check in upon arrival."}</Text>
            <Text style={styles.infoContent}>Structure: {"Central metallic gold VIP digital pass card + detailed summary breakdown below + dual floating action buttons"}</Text>
          </View>

          {/* Interactive UI Component Cards */}
          {["VIP Digital Pass Card with animated gold metallic edge border","Dynamic High-Contrast Check-in QR Code","Appointment Detail Grid (Stylist, Date, Time, Chair #)","Concierge Choices Summary (Beverage, Audio preference)","Add to Digital Wallet Button","Calendar Sync & Location Map Buttons"].map((comp, idx) => {
