...
export const ServiceConciergeScreen: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.card}>
      <Text style={styles.cardTitle}>{"Service Menu & Concierge Preferences Screen"}</Text>
      <Text style={styles.cardDesc}>{"Allows clients to select haircut and beard services while customizing bespoke pre-appointment amenities."}</Text>
      ...
      {viewState === 'LOADING' && (
        <View style={[styles.stateBanner, styles.loadingBanner]}>
          ...
          <Text style={styles.loadingText}>⏳ {"Accordion skeletons and placeholder cards pulsing softly."}</Text>
        </View>
      )}

      {viewState === 'EMPTY' && (
        <View style={[styles.stateBanner, styles.emptyBanner]}>
          <Text style={styles.emptyText}>📭 {"Alert: 'No services available for current selection. Please adjust criteria.'"}</Text>
        </View>
      )}

      {viewState === 'ERROR' && (
        <View style={[styles.stateBanner, styles.errorBanner]}>
          <Text style={styles.errorText}>⚠️ {"Banner: 'Failed to lock service pricing. Please refresh.'"}</Text>
        </View>
      )}

      {viewState === 'SUCCESS' && (
        <View style={[styles.stateBanner, styles.successBanner]}>
          <Text style={styles.successText}>✅ {"Concierge preferences confirmed checkmark toast."}</Text>
        </View>
      )}
      ...
      {viewState === 'NORMAL' && (
        <View style={{ gap: 12 }}>
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoTitle}>{"Curate custom grooming services and specify personal lounge environment preferences (beverage, ambient lighting, music genre)."}</Text>
            <Text style={styles.infoContent}>Structure: {"Step progress header + segmented tab control (Services / VIP Concierge) + scrollable customization list + sticky subtotal footer"}</Text>
          </View>

          {/* Interactive UI Component Cards */}
          {["Service Selection Cards with Duration & Price","Add-on Customizer Toggles (e.g., Scalp Massage, Charcoal Mask)","Concierge Beverage Picker (Single Malt Scotch, Espresso, Sparkling Water)","Ambient Audio/Lighting Mood Selector","Footer Summary Bar with estimated duration and total price"].map((comp, idx) => {
