...
export const ClientVaultScreen: React.FC = () => {
  const [viewState, setViewState] = useState<ViewState>('NORMAL');

  return (
    <div className="card">
      <div className="card-title">{"Concierge Client Vault Screen"}</div>
      <p className="card-desc">{"Manage client grooming history, saved hair/beard formula notes, high-res transformation gallery, VIP tier status, and personal preferences."}</p>
      ...
      {viewState === 'LOADING' && (
        <div className="state-banner loading">
          ⏳ {"Skeleton elements for header card, gray grid tiles for historical haircut photos"}
        </div>
      )}

      {viewState === 'EMPTY' && (
        <div className="state-banner empty">
          📭 {"Empty Vault graphic with message: 'Your Vault is empty. Complete your inaugural booking to unlock style archiving and formula tracking.'"}
        </div>
      )}

      {viewState === 'ERROR' && (
        <div className="state-banner error">
          ⚠️ {"Banner message: 'Vault sync delayed. Pull down to refresh your grooming records.'"}
        </div>
      )}

      {viewState === 'SUCCESS' && (
        <div className="state-banner success">
          ✅ {"Progress bar gold illumination animation when new VIP tier threshold is reached"}
        </div>
      )}
      ...
      {viewState === 'NORMAL' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ background: 'var(--bg-main)', padding: '20px', borderRadius: 'var(--radius)', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginTop: '4px', color: 'var(--text-main)' }}>
                {"Track grooming progress over time, reorder favorite services, and inspect personal VIP rewards."}
              </h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Architecture: {"Header VIP Membership pass card + Status progress indicator + Tabbed vault content (Look Gallery, Cut History, Formula Notes, VIP Perks) + Vertical activity list"}
              </p>
            </div>
          ...
          <div className="grid-2">
            {["VIP Membership Pass Card","Tier Progress Bar","Past Visit Timeline Cards","Style Formula Notes Card","Personal Photo Gallery Grid","Saved Barber Quick-Connect List"].map((comp, idx) => {
