...
export const BookingPickerScreen: React.FC = () => {
  const [viewState, setViewState] = useState<ViewState>('NORMAL');

  return (
    <div className="card">
      <div className="card-title">{"Precision Slot & Concierge Screen"}</div>
      <p className="card-desc">{"Provide exact appointment date/time slot selection and granular concierge customization (beverage, music, conversation preference)."}</p>
      ...
      {viewState === 'LOADING' && (
        <div className="state-banner loading">
          ⏳ {"Horizontal date pill skeletons, pulsing grid tiles for time slots"}
        </div>
      )}

      {viewState === 'EMPTY' && (
        <div className="state-banner empty">
          📭 {"Date state: 'No open slots on selected date with this Master Stylist. Tap to view next available slot on Oct 24.'"}
        </div>
      )}

      {viewState === 'ERROR' && (
        <div className="state-banner error">
          ⚠️ {"Modal dialog alert: 'The selected slot was just reserved by another client. Please choose an alternative time slot.'"}
        </div>
      )}

      {viewState === 'SUCCESS' && (
        <div className="state-banner success">
          ✅ {"Instant transition to full-screen processing overlay upon card/Apple Pay touch completion"}
        </div>
      )}
      ...
      {viewState === 'NORMAL' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ background: 'var(--bg-main)', padding: '20px', borderRadius: 'var(--radius)', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginTop: '4px', color: 'var(--text-main)' }}>
                {"Lock in a precise appointment time slot and tailor every detail of the in-chair experience."}
              </h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                Architecture: {"Top sticky horizontal calendar slider + Scrollable time slot matrix + Accordion group for Bespoke Concierge Preferences + Summary breakdown tray + Primary checkout action button"}
              </p>
            </div>
          ...
          <div className="grid-2">
            {["Calendar Date Strip","Time Slot Selection Matrix","Concierge Preference Switcher","Beverage & Aromatherapy Selector","Booking Summary Drawer","Apple Pay / Card Authorization CTA Button"].map((comp, idx) => {
